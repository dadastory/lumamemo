...
默认情况下，您不需要进行任何配置，LumaMemo 会使用 OpenStreetMap Nominatim 作为默认的位置提供器。
...
LumaMemo 支持两种位置提供器，您可以根据需求选择合适的提供器。

## Nominatim

Nominatim 是 OpenStreetMap 提供的免费逆向地理编码服务。默认情况下，LumaMemo 会使用 Nominatim 作为位置提供器。
...
当您配置了 `NUXT_MAPBOX_ACCESS_TOKEN` 环境变量后，LumaMemo 会自动使用 Mapbox Search API 作为位置提供器。
